...
app.config['SECRET_KEY'] = os.urandom(24)  # توليد مفتاح سري عشوائي
...
app.config['SQLALCHEMY_DATABASE_URI'] = 'postgresql://postgres:OhVWYhFMqQqRdBYyhcLakkmdCsUTajOu@postgres.railway.internal:5432/railway'
...
class Evaluation(db.Model):
    ...
    image_path = db.Column(db.String(255), nullable=True) #مسار الصورة المرسلة من قبل الموظف 
...
def delete_eval_image():
    ...
    if evaluation and evaluation.image_path:
        # تأكد من أن `image_path` يحتوي فقط على اسم الصورة وليس المسار الكامل
        image_path = os.path.join(app.config['UPLOAD_FOLDER'], evaluation.image_path)
...
def delete_evaluation():
    ...
    if evaluation.image_path:
        image_full_path = os.path.join(app.config['UPLOAD_FOLDER'], os.path.basename(evaluation.image_path))
...
def submit_evaluation():
    try:
        ...
        if image:
            ...
            # ثم خزّن المسار في قاعدة البيانات
